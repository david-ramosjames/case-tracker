...
alter table public.case_tracker_entries
  add column if not exists lit_plaintiff_dep_person text not null default '',
  add column if not exists lit_plaintiff_dep_status public.litigation_event_status,
  add column if not exists lit_defendant_dep_person text not null default '',
  add column if not exists lit_defendant_dep_status public.litigation_event_status,
  add column if not exists lit_mediation_person text not null default '',
  add column if not exists lit_mediation_status public.litigation_event_status,
  add column if not exists lit_trial_person text not null default '',
  add column if not exists lit_trial_status public.litigation_event_status;

comment on column public.case_tracker_entries.lit_plaintiff_dep_person is 'Person assigned to plaintiff deposition (litigation events).';
comment on column public.case_tracker_entries.lit_plaintiff_dep_status is 'Plaintiff deposition scheduling status.';
comment on column public.case_tracker_entries.lit_defendant_dep_person is 'Person assigned to defendant deposition (litigation events).';
comment on column public.case_tracker_entries.lit_defendant_dep_status is 'Defendant deposition scheduling status.';
comment on column public.case_tracker_entries.lit_mediation_person is 'Person assigned to mediation (litigation events).';
comment on column public.case_tracker_entries.lit_mediation_status is 'Mediation scheduling status.';
comment on column public.case_tracker_entries.lit_trial_person is 'Person assigned to trial (litigation events).';
...
as $$
  select jsonb_build_object(
    ...
    'lit_plaintiff_dep_person', entry.lit_plaintiff_dep_person,
    'lit_plaintiff_dep_status', entry.lit_plaintiff_dep_status,
    'lit_defendant_dep_person', entry.lit_defendant_dep_person,
    'lit_defendant_dep_status', entry.lit_defendant_dep_status,
    'lit_mediation_person', entry.lit_mediation_person,
    'lit_mediation_status', entry.lit_mediation_status,
    'lit_trial_person', entry.lit_trial_person,
